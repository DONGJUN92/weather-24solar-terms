...
`git archive HEAD` 만으로는 부족하다 — `사무국 추가 공유 자료/`와 `오리엔테이션/`은
git이 추적하는 파일이라 그대로 담긴다(실측 10.2MB). 사무국이 배포한 자료를 사무국에
되돌려 제출하지 않도록 pathspec으로 제외한다. `.gitignore` 대상(.env*·원시 CSV·
__pycache__·.claude/)은 git archive가 구조적으로 뺀다.
...
def main():
    ...
    doc += [
        ...
        "## 이 폴더에 없는 것 — 일부러 뺐습니다",
        "",
        "- `사무국 추가 공유 자료/`·`오리엔테이션/` — 사무국이 배포한 자료입니다. 되돌려 제출하지 않습니다.",
