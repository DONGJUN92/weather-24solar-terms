...
def main():
    ...
    notes(S[0], ['24절기는 태양 위치로 정한 천문 날짜입니다. 움직이지 않습니다.',
                 '그런데 그 무렵의 날씨는 움직입니다. 이 앱은 그 두 축을 학생이 직접 비교하게 합니다.',
                 '지금 이 URL을 열면 로그인 없이 바로 만질 수 있습니다.'])
    print('S1 표지')
    ...
    notes(s, ['1인 팀입니다. 기획부터 배포·검증까지 단독 수행했고, 생성형 AI를 개발 파트너로 상시 썼습니다.',
              '한 미션은 예측 봉인 → 직접 조작 → 내 결론 세 단계입니다. 미션 1은 16화면입니다.',
              '데이터는 기상청 ASOS 종관기상관측 일자료입니다. 16지점 × 24절기 × 3지표.'])
    print('S5 기본정보')
    ...
    notes(s, ['학습 대상은 중3~고2입니다. 세 가지 근거로 골랐습니다.',
              '첫째, 교육과정에서 기후변화와 자료 해석 성취기준이 배치되는 학년입니다.',
              '둘째, 임계값 개념과 평균·빈도 구분이 가능한 최소 학년입니다.',
              '셋째, 24절기를 생활에서 들어는 봤지만 천문 기준임은 배우지 않은 상태라 오개념 교정 여지가 가장 큽니다.',
              '난이도는 용어를 세 단으로 올려 맞췄습니다. 생활어로 시작해 조작어를 거쳐 학술어까지 갑니다. 수식은 끝까지 노출하지 않습니다.',
              '필수 경로는 5개 미션이고, 실험실·전국지도·비교구간은 선택 분기입니다. 교사용 학습지를 함께 제공합니다.'])
    print('S6 학습대상')
    ...
    notes(s, ['문제는 절기·날씨·기후를 섞어 말하는 것입니다. “처서가 더워졌다”는 천문 날짜에 기온을 귀속하는 오개념입니다.',
              '개발계획서에 적은 다섯 개념이 각각 어느 화면에서 다뤄지는지 좌측에 대조했습니다.',
              '확장 개념인 태풍·위성 비교는 학습 초점을 흐리지 않기 위해 배포본에서 제외했고, README에 미사용을 명시했습니다.',
              '우측은 학습자가 체험 중에 주제를 인식하는 지점입니다. 문구는 화면에 실제로 있는 것을 그대로 인용했습니다.',
              '특히 개념 렌즈 마지막 문항은 “절기를 기후에 맞춰 옮기자”는 오개념을 직접 판별합니다.'])
    print('S7 문제정의')
    ...
    notes(s, ['차별화는 실측과 모형을 한 앱에 넣은 것입니다. 2×2로 보면 두 축을 모두 가진 칸이 비어 있었습니다.',
              'PhET·En-ROADS는 모형만 있고 실측 대조가 없습니다. 기후 스트라이프나 기상자료개방포털은 실측만 있고 조작 학습이 없습니다.',
              '24절기를 쓴 이유는 학습자가 이미 알고 있다는 점입니다. 아는 것이라야 “틀렸을 수도 있다”는 반증 동기가 생깁니다.',
              '우측 루프에서 ③번이 핵심입니다. 예측을 봉인만 하는 앱은 많지만, 봉인한 예측을 다시 꺼내 자료와 대조하는 곳은 드뭅니다.',
              '어긋났을 때 “어긋난 지점이 배울 곳입니다”라고 명시합니다.'])
    print('S8 아이디어')
    ...
    notes(s, ['이 표가 이 앱의 핵심입니다. 기준선 하나를 옮기면 아래 숫자가 즉시 다시 계산됩니다.',
              '25도로 보면 더위일이 31일에서 68일로, 더위가 그치는 날이 8월 30일에서 9월 12일로 13일 늦어집니다.',
              '28도로 올리면 격차가 21일로 커집니다. 같은 자료인데 기준을 바꾸면 결론이 달라집니다.',
              '33도까지 올리면 과거·현재 모두 0일이 되어 비교할 것이 남지 않습니다. 그때 앱이 그 자리에서 알려 줍니다.',
              '이것이 “모호한 말은 기준을 정해야 자료가 된다”를 몸으로 배우는 방식입니다.',
              '기준선을 만지지 않으면 판정 버튼이 잠깁니다. 클릭만으로는 통과할 수 없습니다.',
              '우측 지도는 한 지역의 결과를 전국으로 넓힐 수 없다는 것을 눈으로 확인하는 보기입니다.'])
    print('S9 체험설계')
    ...
    notes(s, ['학습 성과는 완료 화면 ‘내 기록’으로 회수됩니다. 교사가 그대로 채점 근거로 씁니다.',
              '배점표가 요구하는 “잘못된 조작·선택에 대한 학습적 피드백”은 여섯 가지로 설계했습니다.',
              '핵심 원칙은 오답에 정답을 바로 알려 주지 않는 것입니다. 1차 오답이면 정답을 감추고 자료로 되돌립니다.',
              '기준을 극단으로 올려 비교가 불가능해지면, 판정을 누르기 전에 조작 화면에서 그 자리에서 알려 줍니다.',
              '내 결론 칸에도 내용 요건이 있습니다. 근거에는 단위가, 한계에는 범위를 가리키는 말이 있어야 통과합니다.',
              '문장을 복사하거나 화면 안내문을 베끼면 걸립니다.'])
    print('S10 학습성과')
    ...
    notes(s, ['데이터는 기상청 ASOS 일자료입니다. 1969년부터 2026년까지, 16지점 3지표입니다.',
              '중요한 것은 사전 계산한 이미지가 아니라는 점입니다. 조작하면 그 자리에서 다시 계산합니다.',
              '자동 회귀 검증 8,360건이 매 커밋 돌아갑니다. 데이터 8,213건과 문서 정합성 147건입니다.',
              '우측은 제품 안의 AI입니다. 일부러 과장한 결론을 넣으면 지역 확대와 인과 단정을 둘 다 잡습니다.',
              '이 응답은 배포본에서 실제로 받은 것입니다.',
              'AI가 실패하면 기기 안 규칙 점검으로 넘어가 학습은 항상 완결됩니다.',
              '제작 과정에서도 AI를 썼습니다. 여섯 차례 AI 레드팀으로 제 결과물을 공격하게 하고 전량 수정했습니다.',
              '프롬프트 세션 원문 6건을 제출물에 넣었습니다.'])
    print('S11 구현완성도')

    notes(S[12], ['질문 주시면 앱을 직접 열어 보여 드리겠습니다.',
                  '기준선을 옮기면 숫자가 어떻게 바뀌는지, 극단으로 올리면 앱이 어떻게 한계를 인정하는지 보실 수 있습니다.'])
