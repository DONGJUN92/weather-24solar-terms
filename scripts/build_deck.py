...
def main():
    ...
    # 멘토링 지적(진입장벽)을 여기서 받는다 — 난이도 적합성의 답이 '쉽게 만들기'가 아니라
    # '입구를 나누기'였다. 두 입구는 앞뒤로 잇는 단계가 아니라 서로 오갈 수 있는 형제 화면이다.
    textbox(s, bx + 0.30, by_ + 3.26, bw - 0.60, 0.24,
            ['■ 입구를 둘로 — 같은 실측 자료를 3분과 두 차시로'], size=12, color=NAVY)
    step_strip(s, bx + 0.30, by_ + 3.58, bw - 0.60, 0.86,
               [('체험', '3분 · 4클릭', '개념 1개 — 처서'),
                ('학습', '5개 미션', '예측 → 조작 → 내 결론'),
                ('선택', '실험실 · 16지점 · 2100년', '심화 분기'),
                ('교사', '학습지', '오개념 표 · 루브릭')])
    ...
    textbox(s, lx + 0.22, ly + 3.36, lw - 0.44, 1.20, [
        ...
        # 멘토링 지적(미래 시나리오)을 조작 축으로 받는다 — 이 앱의 한 문장을 2100년까지 연장한다
        # 길이는 위 세 줄에 맞춘다 — 한 글자만 넘쳐도 '계/절'로 갈라져 상자 밖으로 밀린다(실측)
    ...
    # 8,360건은 절기 데이터 회귀, 31건은 SSP 전망값이 보고서 표와 한 자리도 어긋나지 않음을
    # 앵커로 대조한 것이다. 두 숫자를 나란히 두면 '가져다 쓴 게 아니라 검증했다'가 읽힌다.
    #
    # 첫 로드 용량 정정(2026-08-13): 예전 배지는 '134KB'였는데 그 값은 데이터 파일
    # (solar_terms_data.js·korea_geo.js)을 뺀 앱 셸만 잰 것이었다. 그런데 그 파일들도
    # <script src>로 첫 화면에서 같이 받으므로 '첫 로드'라는 라벨과 맞지 않았다.
    # 실제 전송량을 gzip으로 재서 그대로 적는다 — 자기 숫자에 같은 잣대를 대는 것이 이 앱의 원칙이다.
    # 배지가 5개로 늘어 칸이 좁아졌다 — 라벨을 6자 안쪽으로 줄여야 '검 증'처럼 갈라지지 않는다(실측)
    stat_row(s, lx + 0.28, ly + 3.94, lw - 0.56, 0.62,
             [('8,360건', '회귀 검증'), ('31건', 'SSP 대조'),
              ('0건', '겹침·잘림'), ('277KB', '첫 로드 gzip'), ('0개', '외부 의존')])
    R = by_name(s, '직사각형 10'); rx, ry, rw, rh = geo(R)
    set_lines(R.text_frame, [
    ...
    quote_card(s, rx + 0.26, ry + 0.62, rw - 0.52, 0.62, '학생이 쓴 결론 (일부러 과장)',
               ...
               label_size=10.5, body_size=10)
    # 'OK'로 둔다 — HTTP 상태코드를 근거처럼 읽히게 하지 않는다(발표자가 실물에서 고쳐 둔 표현)
    quote_card(s, rx + 0.26, ry + 1.40, rw - 0.52, 1.02, 'AI 응답 (배포본 실측 · OK)',
    ...
    # 쪽번호는 슬라이드 번호 필드라 PowerPoint 로 열면 알아서 다시 계산된다.
    # 그런데 파일 안에 남아 있는 '캐시된 글자'는 템플릿 시절 번호(5,6,7…)라,
    # PowerPoint 를 거치지 않고 텍스트를 뽑는 도구(제출 점검 스크립트·PDF 변환기)에는
    # 그 옛 숫자가 그대로 보인다. 실제 위치로 다시 적어 파일 자체를 맞춰 둔다.
    # 주의: 이 자리는 일반 런이 아니라 <a:fld>(슬라이드 번호 필드)다.
    # set_lines 로 런을 편집하면 필드 옆에 런이 하나 더 생겨 '5'+'2' = '52'가 찍힌다.
    # 필드는 그대로 두고 그 안의 캐시 글자(a:t)만 실제 위치로 고친다.
    n_fix = 0
    for i, sl in enumerate(prs.slides, start=1):
        for sh in sl.shapes:
            if not (sh.name.startswith('슬라이드 번호 개체 틀') and sh.has_text_frame):
                continue
            for fld in sh.text_frame._txBody.iter(qn('a:fld')):
                t = fld.find(qn('a:t'))
                if t is None:
                ...
                t.text = str(i)
                n_fix += 1
    print('쪽번호 필드 캐시 재계산 → %d개 (1~%d장)' % (n_fix, len(prs.slides._sldIdLst)))
