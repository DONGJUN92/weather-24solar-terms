...
c.drawString(48, 68, "학습 동선 · 미션 하나 핵심 2분 + 선택 심화 · 5미션 + 실험실 핵심만 약 15분 · 심화·자유탐구까지 55~70분")
foot("해법")
...
rows = [
    ("열을 머금는 층  0.5m → 50m", "지연 3일 → 88일", "계절 지연의 원인이 열관성임을 학습자가 만들어 본다", SEA),
    ("온실효과  0 → +12 W/m²", "연평균 12.4 → 14.7℃ · 지연 17일 → 17일", "곡선은 올라가는데 지연은 불변 — 미션5 결론을 모형이 증명", CORAL),
    ("서울 위도 · 두께 5m", "지연 40일 · 최고 28.0℃", "실측(40일 · 29.4℃)과 거의 일치 — 단순한 모형이 리듬을 맞힌다", GREEN),
]
yy = H - 268
...
c.setFont("MG", 11.5); c.setFillColorRGB(*MUTED)
c.drawString(48, 68, "관측 3축(기준값·날짜·기간)은 자료가 무엇을 말하는지를, 물리 1축은 왜 그런지를 다룬다 — 둘 다 필요하다")
foot("모형")

# ── 6. 자기 수치 반증 ─────────────────────────────────────────
c.showPage(); bg()
head("검증", "우리 수치를 우리가 반증", "레드팀 4라운드 자가 감사에서 발견 · 발표 전 전량 수정")
...
c.setFont("MG", 11.5); c.setFillColorRGB(*MUTED)
c.drawString(48, 68, "발견 경로 · AI 레드팀 4라운드 · 3라운드 44건 + 4라운드 40건 적발 → 전량 수정 · AI 주장 11건은 원자료 재검증으로 기각")
foot("검증")

# ── 7. 재발 방지 ──────────────────────────────────────────────
c.showPage(); bg()
head("재발 방지", "같은 실수가 돌아오지 못하게", "검증 스크립트가 배포 전에 차단")
big(72, H - 216, "8,213", "회귀 검증 검사", SEA, 46)
big(300, H - 216, "12축", "구조·표본·단조성·실측대조·부호방향·강수전환·계절지연·극값안정·이동창·문서대조·절기날짜·전지구·안정성", GREEN, 46)
big(72, H - 320, "99/100", "규칙 점검 평가 100케이스 · 오탐 0", SUN, 34)
